...
ggsave('figures/3Spp_WB/rawCounts.png')
...
d2 %>% group_by(riverOrderedGG,seasonGG,year) %>%
  ...
  geom_text(data=tempModsTidy %>% filter(riverOrderedGG == "West Brook"),aes(x=2000.5,y=17.5, label=paste(round(estimate,2),sig))) +
  theme_publication() +
  facet_wrap(~seasonGG)
...
ggsave('figures/3Spp_WB/meanGRYOY1.png', width = 7, height = 7, units='in')
...
ggsave('figures/3Spp_WB/meanGRYOY2.png', width = 7, height = 7, units='in')
...
cd %>% group_by(species) %>% filter(riverOrderedGG == "West Brook") %>%
  summarize(meanGR = mean(grLength, na.rm=T),
            sdGR = sd(grLength, na.rm=T))
.182/.197 #bkt/bnt
.151/.197 #ats/bnt

cd %>% group_by(isYOY) %>% filter(riverOrderedGG == "West Brook") %>%
  summarize(meanGR = mean(grLength, na.rm=T),
            sdGR = sd(grLength, na.rm=T))
.120/.226 #adult/yoy
.215/.119
...
cd %>% group_by(season) %>% filter(riverOrderedGG == "West Brook") %>%
  summarize(meanGR = mean(grLength, na.rm=T),
            sdGR = sd(grLength, na.rm=T))
.437/((.0876+.0866+.0485)/3)
```
...
```{r p(detection)}
  ...
  ggplot(p, aes(year,invlogit(est),color=factor(species))) + geom_point() + facet_grid( season ~ isYOY)
 
  # seem to need to run the ggsave outside of markdown by pasting into console
   ggsave('figures/3Spp_WB/p.png')
...
plotHPD <- function(sppToPlot){
  ...
  #print(c(min(p$intByDayDown,na.rm=T),max(p$intByDayDown,na.rm=T)))
    ...
    gg <- ggplot(p %>% filter(river=='west brook'), aes(x = intByDay, y = reorder(parameter,intByDay,na.rm=TRUE), color = factor(seasonGG), shape = factor(isYOY))) + 
        theme_publication() +  
        geom_point(size = 3) +
...
plotHPD('bkt'); ggsave('figures/3Spp_WB/HDP_bkt.png', height = 3, width = 3, scale = 2)
plotHPD('bnt'); ggsave('figures/3Spp_WB/HDP_bnt.png', height = 3, width = 3, scale = 2)
plotHPD('ats'); ggsave('figures/3Spp_WB/HDP_ats.png', height = 3, width = 3, scale = 2)
```
...
#  p$intByDay <- ifelse(p$isYOY == 1 & p$season == 2, NA, p$intByDay)
 ...
 # p$intByDayUp <- p$intUp/p$meanSampleInterval; p$intByDayDown <- p$intDown/p$meanSampleInterval
  ...
  #print(c(min(p$intDown,na.rm=T),max(p$intDown,na.rm=T)))
    ...
    gg <- ggplot(p %>% filter(river == 'west brook'), aes(x = exp(int), y = reorder(parameter,int,na.rm=TRUE), color = factor(season), shape = factor(isYOY))) + 
        theme_publication() +  
        geom_point(size = 3) +
...
plotHPDSigma('bkt'); ggsave('figures/3Spp_WB/HDPSigma_bkt.png', height = 3, width = 3, scale = 2)
plotHPDSigma('bnt'); ggsave('figures/3Spp_WB/HDPSigma_bnt.png', height = 3, width = 3, scale = 2)
plotHPDSigma('ats'); ggsave('figures/3Spp_WB/HDPsigma_ats.png', height = 3, width = 3, scale = 2)
```
...
#  q[q$median < -0.13,]
    ...
    ggplot(q %>% filter(river == 'west brook'), aes(x = median, y = parameterOrd, color = factor(speciesGG), shape = factor(isYOY))) + 
...
ggsave('figures/3Spp_WB/grBetas.png', height = 4, width = 10, scale = 2)
...
for(i in c('bkt','bnt', 'ats')){
  ...
  ggsave(paste0('figures/3Spp_WBpredGRGrid',"_",i,"_",j,'.png'), width = 9, height = h[[i]], units='in')
...
ggsave('figures/3Spp_WBgrByYearYOY1.png')
...
ggsave('figures/3Spp_WBgrByYearYOY2.png')
