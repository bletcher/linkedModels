...
simData <- list(
  nReps = 25,#100, #num of mcmc iterations to grab
  numInd = 500 , # number of individual to simulate
  nOccYOY = 3,
  nOccAdult = 7,
  riverIndex = 1, #Levels: west brook < wb jimmy < wb mitchell < wb obear
  minSD = -2, #range and step for independent variable values in the sim
  maxSD = 2, 
  step = 1
)
simData$nOcc <- simData$nOccYOY + simData$nOccAdult
simData$indVarSeq = c(-1.5, -0.75, 0, 1.5) #seq(simData$minSD,simData$maxSD,simData$step)
...
save( stats3Spp, lm, relImp, anov, elapsed, simData, file = paste0('D:/projects/linkedModels/data/out/projectionOut_N',simData$nReps,'.RData'))

# if not re-running the sim
# load( file = 'D:/projects/linkedModels/data/out/projectionOut.RData')
# temporary while projOut is a list
# stats3Spp <- projOut[[1]]
# relImpData <- projOut[[3]]
# anovData <- projOut[[4]]
# simData <- projOut[[6]]
# spp <- c('bkt','bnt','ats')
