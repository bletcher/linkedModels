...
#  save(q, file= "data/out/allBetas.RData")  for projections
...
ggsave('figures/3Spp_WB/grBetas.png', height = 4, width = 10, scale = 1.5)
...
for( speciesForProj in spp ){
  ...
  lm[[speciesForProj]] <- lm( lenMean ~ temp + flow + bkt + bnt + ats + temp * flow + temp^2 + flow^2 + bnt^2, data = sizeStats %>% filter(occ == (simData$nOcc)))
...
save( projOut, file = 'D:/projects/linkedModels/data/out/projectionOut.RData')
...
ggplot(rIAll %>% filter(var<7), aes((occ), value, color = factor(rowname))) +
  geom_point() +
  geom_line() +
...
ggplot(rIMeans, aes(factor(var),mean, fill = factor(rowname, levels = c('bktRI','bntRI','atsRI'), ordered = TRUE))) +
  geom_errorbar(aes(ymin = mean, ymax = up), width = 0.25, position=dodge) +
  geom_col(position=dodge) +
  scale_x_discrete('Parameter',
                   labels = c('1' = 'Temp','2' = 'Flow','3' = 'Bkt','4' = 'Bnt','5' = 'Ats','6' = 'Temp*Flow', '7' = "Bkt*Bnt")) +
  scale_fill_grey() +
...
ggplot(cAll %>% filter(var %in% 2:7), aes((occ), value, color = factor(rowname))) +
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 3.5) +
  scale_x_continuous('Sampling occasion',
...
# intercept only
ggplot(cAll %>% filter(var == 1), aes((occ), value, color = factor(rowname))) +
  geom_point(size=4) +
  geom_line() +
  geom_vline(xintercept = 3.5) +
  scale_x_continuous('Sampling occasion',
