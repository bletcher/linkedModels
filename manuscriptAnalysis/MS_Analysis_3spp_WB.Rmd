...
obsLenStats <-  
  ddGIn2[[speciesForProj]] %>%
    ...
    summarise(lenMeanMean = mean(lenMean, na.rm=TRUE),
              lenSDMean = sd(lenMean, na.rm=TRUE)) 

#data for running the simulation
simData <- list(
  nReps = 1000, #num of mcmc iterations to grab
  numInd = 500, # number of individual to simulate
  nOccYOY = 3,
  nOccAdult = 7,
  riverIndex = 1, #Levels: west brook < wb jimmy < wb mitchell < wb obear
  
  species = speciesForProj,
  meanInit = obsLenStats$lenMeanMean,
  sdInit = obsLenStats$lenSDMean,
  nSamplesAvail = mcmcInfo$nChains * mcmcInfo$nSamples
)
