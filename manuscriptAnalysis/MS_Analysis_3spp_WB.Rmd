...
```{r RI across occ - use this for paper GROWTH RATE}
...
occs <- if (ageForRI == 'yoy') 1:2 else 3:10
...
```{r RI by occ - not used for paper GROWTH}
...
for( occSelected in 1:(simData$nOcc - 1)){
  ...
  for(spp1 in spp){
      ...
      lmAll[[spp1]] <- lm( grMean ~ temp + flow + temp * flow + temp^2 + flow^2 + bkt + bnt + ats + bnt * bkt, data = grStats3Spp[[spp1]][[1]] %>% filter(occ == occSelected, temp %in% indepVarVals, flow %in% indepVarVals, bkt %in% indepVarVals, bnt %in% indepVarVals, ats %in% indepVarVals))
  #stats3Spp[[spp1]][[1]] %>% filter(occ == occSelected)    
  ...
  ggplot(rI, aes(factor(var),value, fill = factor(rowname, levels = c('bktRI','bntRI'), ordered = TRUE))) +
    geom_col(position='dodge') +
    scale_x_discrete('Parameter',
...
ggplot(rIAllMinus, aes(var, y = value2, fill = rowname)) +
  ...
  facet_grid(season ~ age, labeller=labeller(season = labelsSeason2))
