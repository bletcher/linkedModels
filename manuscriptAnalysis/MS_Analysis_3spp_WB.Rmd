...
for( speciesForProj in c('bkt','bnt','ats') ){
  ...
  anov[[speciesForProj]] <- anova( lm( lenMean ~ temp + flow + bkt + bnt + ats, data = sizeStats %>% filter(occ == (simData$nOccYOY + simData$nOccAdult))) )
...
ggplot(sizeStats, aes())


matplot(t(sizeMat[2,,]))


ggplot(predLen %>% filter(iter %in% 1:10), aes(occ,len, group=ind)) +
