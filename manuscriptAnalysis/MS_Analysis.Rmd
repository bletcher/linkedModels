...
mcmc <- list()
load("D:/projects/linkedModels/data/out/dG_1530190754_crossValFALSE_bkt1997_Nimble.RData")
...
load("D:/projects/linkedModels/data/out/dG_1530220511_crossValFALSE_bnt1997_Nimble.RData")
mcmc[['bnt']]  <- mcmcProcessed

load("D:/projects/linkedModels/data/out/dG_1530239054_crossValFALSE_ats1997_Nimble.RData")
...
for(i in 1:length(all))
  #https://stackoverflow.com/questions/31993704/storing-ggplot-objects-in-a-list-from-within-loop-in-r
      local({ i <- i
  singleDat[[i]] <- selectSingleVar(pred, all, "meanPredGR", all[i], isYOYGG)
  print(c(i,all[i]))
 # print(str(singleDat[[i]]))
  
  tmp <- singleDat[[i]] %>% filter(riverGG == "West Brook")
  ggSingle[[i]] <<- ggplot( tmp, aes( eval(as.name(all[i])), meanPredGR, color=speciesGG ) ) +
