...
mcmc <- list()
load("D:/projects/linkedModels/data/out/dG_1517338587_bkt1997_Nimble.RData")
mcmc[['bkt']] <- mcmcProcessed
d <- dddG[[1]]

load("D:/projects/linkedModels/data/out/dG_1517344974_bnt1997_Nimble.RData")
mcmc[['bnt']]  <- mcmcProcessed

load("D:/projects/linkedModels/data/out/dG_1517347750_ats1997_Nimble.RData")
mcmc[['ats']]  <- mcmcProcessed

rm(dddG,ddG,dG,mcmcProcessed)
...
# all three species
ggplot( cdMeansByisYOY %>% filter(riverOrdered == 'west brook'), aes(season, meanGr, color = species) ) +
  geom_errorbar(aes(ymin = meanGrDn, ymax = meanGrUp), width = 0.25, position = pd) +
  geom_point( size = 2 ) +
  geom_line()+
  facet_wrap(~isYOY)
...
ggplot( sigmaInt[['bkt']] %>% filter(river %in% c('west brook','wb jimmy')), aes(int)) +
  geom_freqpoly() +
  geom_freqpoly(aes(int),color = 'brown', data = sigmaInt[['bnt']]) +
  geom_freqpoly(aes(int),color = 'lightblue', data = sigmaInt[['ats']]) +
  xlim(-4.5,-1) +
  theme_publication() +
  facet_grid(season ~ river + isYOY)
