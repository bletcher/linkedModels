...
mcmc <- list()
load("D:/projects/linkedModels/data/out/dG_1529763682_bkt1997_Nimble.RData")
...
load("D:/projects/linkedModels/data/out/dG_1529720799_bnt1997_Nimble.RData")
mcmc[['bnt']]  <- mcmcProcessed

load("D:/projects/linkedModels/data/out/dG_1529778009_ats1997_Nimble.RData")
...
len2 <- array2df(mcmc[['bnt']]$sims.list$lengthExp, label.x = "est") %>%
         rename(iter=d1,obs=d2)

inds <- data.frame(ind = dG[[1]]$constants$ind[1:(length(dG[[1]]$constants$ind) - 1)],
                   observedLen = dG[[1]]$data$lengthDATA)
inds$obs <- 1:(length(dG[[1]]$constants$ind) - 1)
inds <-  inds %>%
  group_by(ind) %>%
  mutate(first = min(obs),
         firstObs = obs == first,
         observed = !is.na(observedLen)) %>%
  ungroup()

len1 <- left_join(len2,inds)

# means
lenMean1 <- array2df(mcmc[['bnt']]$mean$lengthExp, label.x = "estMean") %>%
         rename(obs=d1)
...
len %>% filter(ind == 20, !firstObs, iter%%50==0) %>%
  ggplot(aes(obs,est)) +
  geom_point() +
  geom_line(aes(group = iter)) +
  geom_point(aes(obs,estMean), color='red', size=3) +
  geom_point(aes(obs,observedLen), color='blue', size=3) 
...
rHat <- list()
rHat[['bkt']] <- array2df(mcmc[['bkt']]$Rhat$grBeta, levels = list(beta=1:4,isYOY=c(0,1),season=1:4,river=riverOrderedIn[1:4]), label.x="rHat")
rHat[['bnt']] <- array2df(mcmc[['bnt']]$Rhat$grBeta, levels = list(beta=1:4,isYOY=c(0,1),season=1:4,river=riverOrderedIn[1:2]), label.x="rHat")
rHat[['ats']] <- array2df(mcmc[['ats']]$Rhat$grBeta, levels = list(beta=1:4,isYOY=c(0,1),season=1:4,river=riverOrderedIn[1:1]), label.x="rHat")
...
# 
# 
# pred <- pAll %>% group_by(species,flow,temp,cBKT,cBNT,cATS,isYOY,season,river) %>%
