...
mcmc <- list()
load("D:/projects/linkedModels/data/out/dG_1528425682_bkt1997_Nimble.RData")
...
load("D:/projects/linkedModels/data/out/dG_1528410796_bnt1997_Nimble.RData")
mcmc[['bnt']]  <- mcmcProcessed

load("D:/projects/linkedModels/data/out/dG_1528735018_ats1997_Nimble.RData")
...
len %>% filter(ind == 1, !firstObs, iter%%50==0) %>%
  ggplot(aes(obs,est)) +
  geom_point() +
...
rHat <- list()
rHat[['bkt']] <- array2df(mcmc[['bkt']]$Rhat$grBeta, levels = list(beta=1:5,isYOY=c(0,1),season=1:4,river=riverOrderedIn[1:4]), label.x="rHat")
rHat[['bnt']] <- array2df(mcmc[['bnt']]$Rhat$grBeta, levels = list(beta=1:5,isYOY=c(0,1),season=1:4,river=riverOrderedIn[1:2]), label.x="rHat")
