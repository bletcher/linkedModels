...
# as range plot
  ...
  q <- q %>% filter( !(river == 'west brook' & season == 2 & isYOY == 0))
...
for(i in 1:length(all))
 ...
 # print(str(singleDat[[i]]))
  ...
  ggSingle[[i]] <<- ggplot( tmp, aes( eval(as.name(all[i])), meanPredGR, color=riverGG ) ) +
      geom_line( size = 1.225 ) +
      theme_bw() +
      theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(), 
            axis.title.y = element_blank(),
...
```{r 2d denstiy plots of mean predictions}
...
for(i in c('bkt','bnt', 'ats')){
  ...
  ggsave(paste0('figures/predGRGrid',"_",i,"_",j,'.png'), width = 9, height = 7, units='in')
